...
* *Catatan interkoneksi tambahan (edit Edge TIDAK memutus rute, 2026-07-03):* backend kini juga menyediakan `PUT /api/v1/edges/{edge_code}` untuk mengubah nama/koordinat sebuah Virtual Edge. Berbeda dari `DELETE`, **`edge_code` tidak pernah berubah** lewat operasi ini — karena itu `target_parent` yang sudah menunjuk ke kode tersebut tetap valid tanpa aplikasi perlu bereaksi apa pun; hanya jarak/hop yang dihitung ulang pada tick topologi berikutnya (≤5 dtk) mengikuti koordinat barunya. Aplikasi tidak perlu — dan tidak akan — menerima `mesh:routing_update` berisi `null` akibat penyuntingan Edge, hanya akibat penghapusannya.
* *Catatan interkoneksi tambahan (hapus riwayat Sesi tidak berdampak ke protokol node, 2026-07-03):* dashboard kini juga menyediakan `DELETE /api/v1/simulations/{session_id}` untuk membersihkan riwayat sesi lama dari daftar. Backend menolak (409) penghapusan selama sesi masih `is_active`, sehingga jalur satu-satunya menuju penghapusan selalu melewati `POST .../stop` terlebih dahulu — yang **sudah** menyiarkan `session:ended` dan memutus paksa seluruh koneksi node (SRS backend, event `session:ended`). Akibatnya tidak ada node yang bisa tersambung ke sesi yang riwayatnya kemudian dihapus; operasi ini murni administratif sisi dashboard dan tidak menambah kondisi baru yang wajib ditangani mesin Data Link Flutter.
